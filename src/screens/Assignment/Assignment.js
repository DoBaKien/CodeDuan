...
const Assignment = () => {
    ...
    const CotGoiY = (data, index) => (
        <TouchableOpacity style={{ alignItems: "center", }} onPress={() => Alert.alert(`Bạn chọn ${data}`)}>
            <Text style={{ textAlign: "center", fontSize: 16, marginHorizontal: 10 }} >{data}</Text>
        </TouchableOpacity>
    );
                        ...
                        <TableWrapper key={index} style={styles.row}>
                            {
                                rowData.map((cellData, cellIndex) => (
                                    <Cell key={cellIndex} data={
                                        cellIndex === 4 ? CotBtn(cellData, index) : cellData &&
                                            cellIndex === 2 ? CotGoiY(cellData, index) : cellData
...
const styles = StyleSheet.create({
    ...
    text: { textAlign: "center", paddingHorizontal: 5, fontSize: 16 },
